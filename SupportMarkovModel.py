...
def draw_survival_curves_and_histograms(simOutputs_without, simOutputs_with):
    ...
    PathCls.graph_sample_paths(
        ...
        legends=['Without Anticoagulation', 'With Anticoagulation']
    )
    ...
    Figs.graph_histograms(
        ...
        y_label='Counts',
        bin_width=1,
        legend=['Without Anticoagulation', 'With Anticoagulation'],
        transparency=0.6
    )


def print_comparative_outcomes(simOutputs_without, simOutputs_with):
    ...
    increase_survival_time = Stat.DifferenceStatIndp(
        ...
        x=simOutputs_with.get_survival_times(),
        y_ref=simOutputs_without.get_survival_times())
    ...
    # increase in number of stroke under anticoagulation therapy with respect to no therapy
    increase_num_stroke = Stat.DifferenceStatIndp(
        name = "Change in number of stroke",
        x=simOutputs_with.get_if_developed_stroke(),
        y_ref= simOutputs_without.get_if_developed_stroke())
    # estimate and CI
    estimate_CI = F.format_estimate_interval(
        estimate=increase_num_stroke.get_mean(),
        interval= increase_num_stroke.get_t_CI(alpha=Settings.ALPHA),
        deci=2)
    print("Average change in number of stroke "
          "and {:.{prec}%} confidence interval:".format(1 - Settings.ALPHA, prec=0),
          estimate_CI)
    ...
    increase_discounted_cost = Stat.DifferenceStatIndp(
        ...
        x=simOutputs_with.get_costs(),
        y_ref=simOutputs_without.get_costs())
    ...
    increase_discounted_utility = Stat.DifferenceStatIndp(
        name='Increase in discounted cost',
        x=simOutputs_with.get_utilities(),
        y_ref=simOutputs_without.get_utilities())
...
def report_CEA_CBA(simOutputs_without, simOutputs_with):
    """ performs cost-effectiveness and cost-benefit analyses
    :param simOutputs_without: output of a cohort simulated under without therapy
    :param simOutputs_with: output of a cohort simulated under Anticoagulation therapy
    """

    # define two strategies
    without_therapy_strategy = Econ.Strategy(
        name='Without Therapy',
        cost_obs=simOutputs_without.get_costs(),
        effect_obs=simOutputs_without.get_utilities()
    )
    with_therapy_strategy = Econ.Strategy(
        name='Anticoagulation Therapy',
        cost_obs=simOutputs_with.get_costs(),
        effect_obs=simOutputs_with.get_utilities()
    )
